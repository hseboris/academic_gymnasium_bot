...
async def start_handler(msg: types.Message):
    ...
    if tg_check and comment_check and vk_check:
        ...
        errors = []
        if not tg_check:
            errors.append("Telegram подписка ❌")
        if not comment_check:
            errors.append("Комментарий под постом ❌")
        if not vk_check:
            errors.append("VK подписка ❌")
        await msg.answer("❗ Не выполнены условия:\n" + "\n".join(errors))

@router.message(F.text == "/participants")
async def list_participants(msg: types.Message):
    if msg.from_user.id not in ADMIN_IDS:
        return  # ничего не отвечаем и игнорируем

    if os.path.exists(PARTICIPANTS_FILE):
        await msg.answer_document(types.FSInputFile(PARTICIPANTS_FILE))
    else:
        await msg.answer("Список участников пока пуст.")
